...
# 加载 E2 checkpoint（多 SNR 训练的最佳权重）
ckpt_path = Path('outputs/phase9/nhfae_e2_multisnr_dynamic/ckpt/best.pt')
...
print(f"✅ Loaded E2 checkpoint from {ckpt_path}\n")

def inference_on_bucket(bucket_name: str, noisy_dir_path: Path):
    """推理指定 SNR 桶中的所有音频。"""
    out_dir = Path(f'outputs/phase9/nhfae_e2_multisnr_dynamic/wav_{bucket_name}')
    ...
    with torch.no_grad():
        for wav_file in sorted(noisy_dir_path.glob('*.wav')):
            ...
            # ===== 修正：需要缩放 magnitude =====
            mag_enhanced = outputs['mag_mix'] * mag_scale  # 恢复到原始范围

...
class MCDMetric(BaseMetric):
    ...
    def __init__(self, fs: int) -> None:
        ...
        self.fs = fs
        # Lazy import to avoid side effects for runs that do not use MCD.
        from mel_cepstral_distance import compare_audio_arrays as mel_cepstral_distance

        self._mcd_fn = mel_cepstral_distance

    @override
    def compute(self, x: np.ndarray, y: np.ndarray) -> float:
        mcds = [self._mcd_fn(x_i, y_i, self.fs, self.fs)[0] for x_i, y_i in zip(x, y)]
...
class NISQAMetric(BaseMetric):
    ...
    @override
    def compute(self, x: np.ndarray, y: np.ndarray) -> float:
        from torchmetrics.functional.audio import non_intrusive_speech_quality_assessment
...
class LPSMetric(BaseMetric):
    ...
    def _lps(self, x: str, y: str) -> float:
        ...
        if len(y) == 0:
            return float("nan")
        lev_distance = self._levenshtein.distance(x, y)

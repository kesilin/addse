...
class DynamicMixingDataset(ld.ParallelStreamingDataset):
    ...
    def __init__(
        ...
        noise_time_stretch_range: tuple[float, float] = (1.0, 1.0),
        noise_shift_prob: float = 0.0,
        noise_selfmix_prob: float = 0.0,
        noise_filter_prob: float = 0.0,
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            noise_aug_prob: Probability of applying noise augmentation.
            noise_time_stretch_range: Time-stretch range for noise augmentation.
            noise_shift_prob: Probability of circular time shift in noise augmentation.
            noise_selfmix_prob: Probability of self-mixing augmented noise.
            noise_filter_prob: Probability of applying a simple FIR filter to the noise.
        ...
        super().__init__(
            ...
            transform=functools.partial(
                ...
                noise_time_stretch_range=noise_time_stretch_range,
                noise_shift_prob=noise_shift_prob,
                noise_selfmix_prob=noise_selfmix_prob,
                noise_filter_prob=noise_filter_prob,
            ),
            length=length,
    ...
    def augment_noise(
        ...
        target_length: int,
        rngs: dict[str, Any],
        noise_aug_prob: float,
        noise_time_stretch_range: tuple[float, float],
        noise_shift_prob: float,
        noise_selfmix_prob: float,
        noise_filter_prob: float,
    ) -> torch.Tensor:
        """Apply lightweight stochastic augmentation on noise.

        The augmentation is intentionally simple and CPU-friendly to increase noise diversity when only a few
        recordings are available.
        """
        if noise_aug_prob <= 0.0 or rngs["random"].random() >= noise_aug_prob:
            return noise[:, torch.arange(target_length) % noise.shape[-1]]

        # 1) Time stretch/compress via interpolation.
        min_stretch, max_stretch = noise_time_stretch_range
        if min_stretch > 0 and max_stretch > 0 and (min_stretch != 1.0 or max_stretch != 1.0):
            factor = rngs["random"].uniform(min_stretch, max_stretch)
            stretched_len = max(8, int(noise.shape[-1] * factor))
            noise_3d = noise.unsqueeze(0)
            noise = F.interpolate(noise_3d, size=stretched_len, mode="linear", align_corners=False).squeeze(0)

        # 2) Circular shift to decorrelate local temporal patterns.
        if noise_shift_prob > 0.0 and rngs["random"].random() < noise_shift_prob:
            shift = rngs["random"].randint(0, max(1, noise.shape[-1] - 1))
            noise = torch.roll(noise, shifts=shift, dims=-1)

        # 3) Self-mix with an independently shifted copy.
        if noise_selfmix_prob > 0.0 and rngs["random"].random() < noise_selfmix_prob and noise.shape[-1] > 1:
            shift2 = rngs["random"].randint(1, max(1, noise.shape[-1] - 1))
            alpha = rngs["random"].uniform(0.3, 0.7)
            noise = alpha * noise + (1.0 - alpha) * torch.roll(noise, shifts=shift2, dims=-1)

        # 4) Randomly apply a simple low-pass or high-pass FIR filter.
        if noise_filter_prob > 0.0 and rngs["random"].random() < noise_filter_prob:
            filt_type = "low" if rngs["random"].random() < 0.5 else "high"
            if filt_type == "low":
                kernel = torch.tensor([0.2, 0.6, 0.2], dtype=noise.dtype, device=noise.device)
            else:
                kernel = torch.tensor([-0.5, 1.0, -0.5], dtype=noise.dtype, device=noise.device)
            noise = F.conv1d(noise.unsqueeze(0), kernel.view(1, 1, -1), padding=1).squeeze(0)
    ...
    def transform(
        ...
        noise = DynamicMixingDataset.augment_noise(
            noise,
            speech.shape[-1],
            rngs,
            noise_aug_prob,
